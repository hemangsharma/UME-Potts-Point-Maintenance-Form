...
<body>
    <div class="container">
        ...
        <h3>MAILS</h3>
        <ul>
            <li>You can find your new mails on ground floor.</li>
...
def load_room_list_data():
    """
    Load room list CSV data expected to have columns:
    'Unit', 'Resident Name', 'Second Resident Name'
    """
    try:
        df = pd.read_csv('room_list.csv')
        # Drop rows where "Resident Name" is empty
...
def render_room_list_page():
    # Load room data
    rooms = load_room_list_data()
    # Generate HTML rows from the room data
    room_rows = ""
    if rooms:
        for room in rooms:
            unit = room.get("Unit", "")
            res1 = room.get("Resident Name", "")
            res2 = room.get("Second Resident Name", "")
            room_rows += f"<tr><td>{unit}</td><td>{res1}</td><td>{res2}</td></tr>"
    else:
        room_rows = "<tr><td colspan='3'>No data available</td></tr>"

    # Replace the Jinja-style loop with the generated rows.
    rendered_html = room_list_template.replace("<!-- ROOM_ROWS_PLACEHOLDER -->", room_rows)
    return rendered_html

# ---------------------------
# Streamlit App Initialization
# ---------------------------
st.set_page_config(page_title="Property Notice Board", page_icon="🏢")
...
if page == 'Home':
    st.title('Home')
    components.html(home_template, height=1200, scrolling=True)

elif page == 'Room List':
    st.title('Room List')
    room_list_html = render_room_list_page()
    components.html(room_list_html, height=1200, scrolling=True)

elif page == 'House Rules':
    st.title('House Rules')
    components.html(house_rules, height=1200, scrolling=True)

elif page == 'Departure Guidelines':
    st.title('Departure Guidelines')
    components.html(departure_guide, height=800, scrolling=True)

elif page == 'Emergency Contacts':
    st.title('Emergency Contacts')
    components.html(emergency_contacts, height=1000, scrolling=True)

elif page == 'Floor Information':
    st.title('Floor Information')
    floors = {
        'Floor 1': 'Reception, Lounge',
        'Floor 2': 'Rooms 101-110',
        'Floor 3': 'Rooms 201-210'
    }
    for floor, description in floors.items():
        st.write(f'🏢 **{floor}:** {description}')

elif page == 'Maintenance Form':
    st.title('Maintenance Request Form')
    components.html(maintenance_form_template, height=1000, scrolling=True)

elif page == 'Guest Form':
    st.title('Guest Request Form')
    components.html(guest_form_template, height=1000, scrolling=True)

elif page == 'About':
    st.title('About')
    st.markdown("""
    This Streamlit dashboard displays property information, including room lists, house rules, departure guidelines, emergency contacts, and forms for maintenance and guest requests.
    
    **Note:** The original Flask submission endpoints are not available in this Streamlit version.
    """)
